...
class OptimTracker(nn.Module):
    ...
    def forward(self, train_imgs, test_imgs, train_bb, test_proposals, train_label, is_distractor=None, test_label=None, test_anno=None):
        ...
        # Concat the RGB and TIR channels
        
        train_feat = self.extract_backbone_features(torch.cat((train_imgs[:3,...],train_imgs[3:,:,1,...].unsqueeze(2)),2).view(-1, train_imgs.shape[-3]+1, train_imgs.shape[-2], train_imgs.shape[-1]))
        test_feat = self.extract_backbone_features(torch.cat((test_imgs[:3,...],test_imgs[3:,:,1,...].unsqueeze(2)),2).view(-1, test_imgs.shape[-3]+1, test_imgs.shape[-2], test_imgs.shape[-1]))
    ...
    def extract_features(self, im, layers):
        if 'classification' not in layers:
            return self.feature_extractor(im[:,:4,...], layers)
        backbone_layers = sorted(list(set([l for l in layers + [self.classification_layer] if l != 'classification'])))
        all_feat = self.feature_extractor(im[:,:4,...], backbone_layers)
...
def steepest_descent_learn_filter_resnet50_newiou(filter_size=1, optim_iter=3, optim_init_step=1.0, optim_init_reg=0.01, output_activation=None,
    ...
    # backbone
    backbone_net = backbones.resnet50_comb4(pretrained=backbone_pretrained)
    ...
    if usepretrain:
        if updback:
            # update backbone
            pretrainmodel['feature_extractor.conv1.weight']=torch.cat((pretrainmodel['feature_extractor.conv1.weight'],pretrainmodel['feature_extractor.conv1.weight'].sum(1).unsqueeze(1)),1)
