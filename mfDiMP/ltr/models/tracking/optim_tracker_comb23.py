...
class OptimTracker(nn.Module):
    def __init__(self, feature_extractor, feature_extractor_i, classifier, bb_regressor, classification_layer, bb_regressor_layer, train_feature_extractor=True):
        super().__init__()
    ...
    def forward(self, train_imgs, test_imgs, train_bb, test_proposals, train_label, is_distractor=None, test_label=None, test_anno=None):
       ...
       #test_feat_clf = torch.cat((test_feat[self.classification_layer], test_feat_i[self.classification_layer]), 1)
        ...
        train_feat_iou = [torch.cat((train_feat[l], train_feat_i[l]), 1).view(num_train_images, num_sequences, train_feat[l].shape[-3]*2, train_feat[l].shape[-2],
                                    train_feat[l].shape[-1]) for l in self.bb_regressor_layer]
        test_feat_iou = [torch.cat((test_feat[l], test_feat_i[l]), 1).view(num_test_images, num_sequences, test_feat[l].shape[-3]*2, test_feat[l].shape[-2],
...
def steepest_descent_learn_filter_resnet50_newiou(filter_size=1, optim_iter=3, optim_init_step=1.0, optim_init_reg=0.01, output_activation=None,
    ...
    net = OptimTracker(feature_extractor=backbone_net, feature_extractor_i=backbone_net_i, classifier=classifier, bb_regressor=bb_regressor,
