...
#        self.im = im
    ...
    def init_classifier(self, x):
        ...
        #self.target_classifier.filter_initializer.filter_conv.weight = torch.nn.Parameter(torch.cat((self.target_classifier.filter_initializer.filter_conv.weight,
        #                                                                                self.target_classifier.filter_initializer.filter_conv.weight),1))
        # Get target boxes for the different augmentations
        ...
        if 'Simple' in self.classifier_net_name:
            ...
            with torch.no_grad():
                self.target_filter1, losses = self.target_classifier.get_filter(x[0][:,:int(x[0].shape[1]/2),:,:].contiguous(), target_boxes, y[0], num_iter=num_iter,
                                                                                   compute_losses=plot_loss, train_bb=target_boxes)
                self.target_filter2, losses = self.target_classifier.get_filter(x[0][:,int(x[0].shape[1]/2):,:,:].contiguous(), target_boxes, y[0], num_iter=num_iter,
                                                                                   compute_losses=plot_loss, train_bb=target_boxes)
                self.target_filter = torch.cat((self.target_filter1, self.target_filter2), 1)
